...
local function handler(msg, editbox)
	...
	if (arg ~= nil and arg ~= "" and UnitIsConnected(arg) and IRT_InnervateEnabled) then
		local code = C_ChatInfo.SendAddonMessage("IRT_INNERVATE", UnitName("player"), "WHISPER", arg);
		if (code ~= 0) then
			IRT_DebugMessage(L.ERROR_ADDON_MESSAGE_WHISPER .. IRT_GetAddonResultMessage(code), "debug", L.OPTIONS_INNERVATE_TITLE);
		end
	end
end
