...
local watcherButton = CreateFrame("CheckButton", "IRT_ReadyCheckWatcherCheckButton", IRT_ReadyCheckOptions, "UICheckButtonTemplate");
...
watcherButton:HookScript("OnClick", function(self)
	if self:GetChecked() then
		IRT_ReadyCheckWatcher = true;
		PlaySound(856);
	else
		IRT_ReadyCheckWatcher = false;
		PlaySound(857);
	end
end);

local watcherText = IRT_ReadyCheckOptions:CreateFontString(nil, "ARTWORK", "GameFontHighlight");
...
IRT_ReadyCheckOptions:SetScript("OnShow", function(self)
	...
	watcherButton:SetChecked(IRT_ReadyCheckWatcher);
...
if (subcategory ~= nil) then
	local subcategoryIO, layout = Settings.RegisterCanvasLayoutSubcategory(subcategory, IRT_ReadyCheckOptions, L.OPTIONS_READYCHECK_TITLE);
end